...
- ⚠️ **AUDIT-52 · Mail-deliverability: geen SPF/DKIM/DMARC op `aidatim.nl`.** Geverifieerd 2026-07-21 (`dig` + 8.8.8.8): nul TXT-records. `mail:test` bevestigt dat verzenden via Google Workspace (MX=smtp.google.com) wérkt, maar externe ontvangers markeren mail als spam / weigeren 'm. **Eigenaar-actie (DNS in Cloudflare):** SPF `@ TXT v=spf1 include:_spf.google.com ~all`; DKIM aanzetten in Workspace-admin + het gegenereerde TXT-record plaatsen; DMARC `_dmarc TXT v=DMARC1; p=none; rua=mailto:info@aidatim.nl`.
